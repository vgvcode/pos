...
class CatalogTable:
    ...
    def __init__(self, endpoint = "http://localhost:8000"):
        ...
        self.__table = self.__dynamodb.Table(CatalogTable.__tableName)
    ...
    def deleteAllItemsFromCatalog(self, catalogId):
...
class Catalog:
    ...
    def copyFromRemote(self, remoteEndpoint):
        'copy remote catalog to local and return local catalog'
        result = True
        try:
            'fetch the remote catalog'
            remoteCat = Catalog(self.__catalogId, endpoint=remoteEndpoint)

            'delete the items in the local catalog'
            ct = CatalogTable()
            ct.deleteAllItemsFromCatalog(self.__catalogId)
            
            'delete the items from in memory catalog'
            self.__items = {}

            remoteItems = remoteCat.getItems()
            
            for i in remoteItems:
                itemId = i
                info = remoteItems[i]
    ...
    def removeRemoteCatalog(self, remoteEndpoint):
        'remove all item from remote catalog. Do no remove from local catalog first as it will diverge local catalogs from remote over time'
        remoteDynamoDb = boto3.resource('dynamodb', endpoint_url = remoteEndpoint)
        remoteTable = remoteDynamoDb.Table(Catalog.__tableName)
        result = False
        try:
